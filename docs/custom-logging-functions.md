...
```javascript
errsole.info('Logging a message');
errsole.info('Multiple', 'arguments', 'are supported');
errsole.info('Logging with a variable:', var1);
errsole.info(new Error('An error occurred'));
errsole.info('Logging with an error object:', errorObject);
```
