...
function validateAndSanitizePath (basePath, reqPath) {
  try {
    const sanitizedPath = new URL(reqPath, `http://${basePath}`).pathname;
...
Errsole.nestExpressProxyMiddleware = function (path, req, res, next) {
  const sanitizedPath = validateAndSanitizePath('localhost:' + this.port, req.url.replace(path, ''));
  const options = {
    hostname: 'localhost',
    port: this.port,
    path: sanitizedPath,
