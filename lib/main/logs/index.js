...
const logCollector = {
  ...
  logCustomMessage (level, message, metadata, errsoleId, timestamp) {
    ...
    try {
      ...
      let output;
      let originalWriteFn;
      // Choose stderr for "alert", "error", and "warn"
      if (['alert', 'error', 'warn'].includes(level)) {
        output = process.stderr;
        originalWriteFn = this.originalStderrWrite;
      } else {
        // Otherwise, default to stdout (e.g. "info", "debug")
        output = process.stdout;
        originalWriteFn = this.originalStdoutWrite;
      }
      // Print to the original stream
      originalWriteFn.call(output, `${message}\n`, 'utf8');
    } catch (err) {
      console.error(err);
    }
  }
