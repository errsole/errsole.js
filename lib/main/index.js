...
const helpers = require('../main/server/utils/helpers');
...
const Main = {
  ...
  async emailIntegration (integrations) {
    try {
      ...
      const requiredFields = ['sender', 'host', 'port', 'username', 'password', 'recipients'];
      for (const field of requiredFields) {
        ...
        if (
          ...
          throw new Error(`The required field "${field}" for email integration is either missing or has no value.`);
        }
      }

      const storage = await storageConnection.getStorageConnection();
      const configResponse = await storage.getConfig('emailIntegration');
      const existingConfig = configResponse?.item?.value;
      let existingReceivers = [];

      try {
        const parsed =
          typeof existingConfig === 'string' ? JSON.parse(existingConfig) : existingConfig;

        if (parsed && parsed.receivers) {
          existingReceivers = parsed.receivers.split(',').map(r => r.trim());
        }
      } catch (err) {
        throw new Error('Failed to parse existing emailIntegration config:', err);
      }
      const newReceivers = Array.isArray(email.recipients)
        ? email.recipients.map(r => r.trim())
        : email.recipients.split(',').map(r => r.trim());

      const mergedReceivers = [...new Set([...existingReceivers, ...newReceivers])];
      const emailData = {
        sender: email.sender,
        host: email.host,
        port: email.port,
        ...
        receivers: mergedReceivers.join(','),
        status: true
      };
  ...
  async slackIntegration (integrations) {
    try {
      ...
      const url = integrations.slack.url;

      if (typeof url !== 'string' || url.trim() === '') {
        throw new Error('The Slack integration URL is either missing or empty.');
      }

      const validateUrl = await helpers.SlackUrl(url);
      if (!validateUrl) {
        throw new Error('Invalid Slack URL provided.');
      }
