    ...
    try {
      ...
      for (const field of requiredFields) {
        ...
        if (
          ...
          throw new Error(`Email integration skipped: "${field}" is missing or empty.`);
        }
      }
      ...
      const emailData = {
        ...
        receivers: Array.isArray(email.recipients) ? email.recipients.join(',') : email.recipients,
        status: true
      };

      await storage.setConfig('emailIntegration', JSON.stringify(emailData));
    } catch (error) {
      throw new Error('Error storing email integration:', error);
    }
  },
  ...
  async slackIntegration (integrations) {
    try {
      ...
      if (typeof url !== 'string' || url.trim() === '') {
        throw new Error('Slack integration skipped: "url" is missing or empty.');
      }
      ...
      throw new Error(`Error storing Slack integration: ${error.message}`);
    }
  }
