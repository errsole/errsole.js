...
const storageConnection = require('../main/server/storageConnection');
...
const Main = {
  ...
  initialize (options) {
    ...
    if (options.storage && typeof options.storage.on === 'function') {
      options.storage.on('ready', () => {
        this.emailIntegration(options.integrations);
      });
    }
    if (options.storage && typeof options.storage.on === 'function') {
      options.storage.on('ready', () => {
        this.slackIntegration(options.integrations);
      });
    }
    ...
    signalEvents.forEach(async (signalEvent) => {
      process.on(signalEvent.key, async () => {
        if (self.isInitializated) {
          console.log(`Gracefully closing on ${signalEvent.key} – Flushing logs before exit...`);
          const messageExtraInfo = {
  ...
  async emailIntegration (integrations) {
    try {
      if (!integrations || !integrations.email) return;
      const storage = await storageConnection.getStorageConnection();
      const emailData = { ...integrations.email };
      if (emailData.recipients) {
        emailData.receivers = Array.isArray(emailData.recipients)
          ? emailData.recipients.join(',')
          : emailData.recipients;
        delete emailData.recipients;
      }
      emailData.status = true;
      await storage.setConfig('emailIntegration', JSON.stringify(emailData));
    } catch (error) {
      console.error('Error storing integrations:', error);
    }
  },

  async slackIntegration (integrations) {
    try {
      if (!integrations || !integrations.slack || !integrations.slack.url) return;

      const storage = await storageConnection.getStorageConnection();
      const slackData = {
        url: integrations.slack.url,
        username: 'Errsole',
        icon_url: 'https://avatars.githubusercontent.com/u/84983840',
        status: true
      };

      await storage.setConfig('slackIntegration', JSON.stringify(slackData));
    } catch (error) {
      console.error('Error storing integrations:', error);
    }
  }
