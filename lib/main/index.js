    ...
    signalEvents.forEach(async (signalEvent) => {
      process.on(signalEvent.key, async () => {
        if (self.isInitializated) {
          ...
          Logs.logCustomMessage('alert', `The application has stopped with the signal ${signalEvent.key} - ${signalEvent.value}`, '{}', errsoleLogId);
          await Alerts.customLoggerAlert(`The application has stopped with the signal ${signalEvent.key} - ${signalEvent.value}`, messageExtraInfo, errsoleLogId);
          await Logs.flushLogs();
        }
        process.exit(0);
      });
