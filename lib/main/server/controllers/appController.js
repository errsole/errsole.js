...
exports.testSlackNotification = async (req, res) => {
  try {
    const result = await Alerts.testSlackAlert('Hello, This is a test notification.', 'Test');
...
exports.testEmailNotification = async (req, res) => {
  try {
    const result = await Alerts.testEmailAlert('Hello, This is a test notification.', 'Test');
    res.send(Jsonapi.Serializer.serialize(Jsonapi.AppType, { success: result }));
  } catch (error) {
    console.error(error);
    res.status(500).send({
