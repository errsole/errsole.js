  ...
  try {
    ...
    if (userCountResult && userCountResult.count !== 0) {
      ...
      if (createUserResult && createUserResult.item) {
        ...
        const errorData = [{
          error: 'Internal Server Error',
          message: createUserResult ? createUserResult.error : 'An internal server error occurred'
        }];
        res.status(500).send({ errors: errorData });
      }
    }
  ...
  try {
    ...
    if (email && password) {
      ...
      if (verifyUserResult && verifyUserResult.item && verifyUserResult.item.email === email) {
        ...
        const errorData = [{
          error: 'Unauthorized',
          message: verifyUserResult ? verifyUserResult.error : 'Login failed, please check your credentials'
        }];
  ...
  try {
    ...
    if (email) {
      ...
      if (userDetailsResult && userDetailsResult.item && userDetailsResult.item.email) {
        ...
        const errorData = [{
          error: 'Internal Server Error',
          message: userDetailsResult ? userDetailsResult.error : 'An internal server error occurred'
        }];
        res.status(500).send({ errors: errorData });
      }
    } else {
  ...
  try {
    ...
    if (email) {
      ...
      if (userDetailsResult && userDetailsResult.item && userDetailsResult.item.email === email) {
        ...
        const errorData = [{
          error: 'Internal Server Error',
          message: userDetailsResult ? userDetailsResult.message : 'An internal server error occurred'
        }];
        res.status(500).send({ errors: errorData });
      }
    } else {
