...
function blockKit (message, type, messageExtraInfo = {}, alertUrl, todayCount) {
  ...
  // Add alert occurrence information
  if (type === 'Alert') {
    payload.blocks.push({
      type: 'section',
      text: {
        type: 'mrkdwn',
        text: `*Note:* This alert has occurred ${todayCount} time${todayCount > 1 ? 's' : ''} today.\n• You will not receive another notification for this alert on this server within the current hour.\n• Errsole uses the UTC timezone in notifications.`
      }
    });
  } else {
    payload.blocks.push({
      ...
      text: {
        type: 'mrkdwn',
        text: `*Note:*\n• This error has occurred ${todayCount} time${todayCount > 1 ? 's' : ''} today.\n• You will not receive another notification for this error on this server within the current hour.\n• Errsole uses the UTC timezone in notifications.`
      }
    });
  }
  ...
  try {
    ...
    if (this.transporter !== null) {
      ...
      if (data && data.item) {
        ...
        if (alertUrl) {
          message = `${message}<br/><a href="${alertUrl}">Click here</a> to view the logs in the Errsole dashboard.<br/><br/>`;
        }

        // Add alert occurrence and notification message
        if (type === 'Alert') {
          message = `${message}<br/><i><b>Note:</b> This alert has occurred ${todayCount} time${todayCount > 1 ? 's' : ''} today.<ul><li>You will not receive another notification for this alert on this server within the current hour.</li><li>Errsole uses the UTC timezone in notifications.</li></ul></i><br/><br/>`;
        } else {
          message = `${message}<br/><i><b>Note:</b> This error has occurred ${todayCount} time${todayCount > 1 ? 's' : ''} today.<ul><li>You will not receive another notification for this error on this server within the current hour.</li><li>Errsole uses the UTC timezone in notifications.</li></ul></i><br/><br/>`;
        }
