...
SlackService.sendAlert = async function (message, type, messageExtraInfo, errsoleLogId) {
  try {
    ...
    if (data && data.item) {
      ...
      const isDuplicateAlert = await checkAlertStatus(message, messageExtraInfo, errsoleLogId);
      if (isDuplicateAlert) {
        return false;
      }
      // create alert url
    ...
    try {
      ...
      if (insertAlertResult) {
        ...
        if (previousItem) {
          ...
          if (
            now.getFullYear() === previousTime.getFullYear() &&
            now.getMonth() === previousTime.getMonth() &&
            now.getDate() === previousTime.getDate() &&
            now.getHours() === previousTime.getHours()
          ) {
