    ...
    const renderConsoleCombinedLogs = () => {
      return combinedLogs.map((log) => {
        ...
        if (level === 'error' || level === 'alert') {
          header = <p className={(meta && meta !== '{}' && 'log_panel_header meta-log') || 'log_panel_header'}><span className='log_timestamp'>{occurredAt}</span><span className='log_message_top' style={{ color: '#db4e09' }}>{message}</span></p>;
        } else {
          header = <p className={(meta && meta !== '{}' && 'log_panel_header meta-log') || 'log_panel_header'}><span className='log_timestamp'>{occurredAt}</span><span className='log_message_top'>{message}</span></p>;
        }
