...
/* Ante UI */
import { Layout, Icon, Collapse, Spin, notification } from 'antd';
...
class ConsoleCombinedLogs extends React.Component {
  ...
  componentDidMount () {
    ...
    if (errorLogTimestamp) {
      this.getConsoleCombinedLogs({ logStartDatetime: errorLogTimestamp, logOrder: 'old' });
    }
  }
  ...
  loadMoreErrors (logOrder) {
    ...
    if (combinedLogs.length > 0 && logOrder === 'latest') {
      ...
      const timestamp = combinedLogs[combinedLogs.length - 1].attributes.timestamp;
      this.getConsoleCombinedLogs({ logStartDatetime: timestamp, logOrder, logId });
    }
    if (combinedLogs.length > 0 && logOrder === 'old') {
      const logId = combinedLogs[0].id;
      const timestamp = combinedLogs[0].attributes.timestamp;
      this.getConsoleCombinedLogs({ logStartDatetime: timestamp, logOrder, logId });
    }
  }
  ...
  render () {
    ...
    const renderConsoleCombinedLogs = () => {
      return combinedLogs.map((log) => {
        ...
        occurredAt = timezone === 'Local' ? moment(occurredAt).format('MMM D, YYYY hh:mm:ss A Z') : moment.utc(occurredAt).format('MMM D, YYYY hh:mm:ss A Z');
        ...
        const panel = <Panel className='log_panel' header={header} key={logId}><pre className='log_message_detail'>{message}</pre></Panel>;
