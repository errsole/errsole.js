...
/* Ante UI */
import { Layout, Icon, Collapse, Dropdown, Button, Spin, notification, Modal, Menu, message } from 'antd';
    ...
    const renderConsoleCombinedLogs = () => {
      return combinedLogs.map((log) => {
        ...
        if (level === 'error' || level === 'alert') {
          header = <p className={(meta && meta !== '{}' && 'log_panel_header meta-logs') || 'log_panel_header'}><span className='log_timestamp'>{occurredAt}</span><span className='log_message_top' style={{ color: '#db4e09' }}>{message}</span></p>;
        } else {
          header = <p className={(meta && meta !== '{}' && 'log_panel_header meta-logs') || 'log_panel_header'}><span className='log_timestamp'>{occurredAt}</span><span className='log_message_top'>{message}</span></p>;
        }
        ...
        const panel = <Panel className={selectedError ? 'log_panel selected_error_log_panel' : 'log_panel'} header={header} key={logId} extra={meta && meta !== '{}' && <span className='view-log-extra' onClick={event => { event.stopPropagation(); }}><Dropdown overlay={menu} className='view-log-details'><Button type='primary'><Icon type='more' /></Button></Dropdown></span>}><pre className='log_message_detail'>{message}</pre><p className='details'><b>Hostname: </b>{hostname} &nbsp;&nbsp;|&nbsp;&nbsp; <b>PID: </b>{pid} &nbsp;&nbsp;|&nbsp;&nbsp; <b>Source: </b>{source} &nbsp;&nbsp;|&nbsp;&nbsp; <b>Level: </b>{level}</p></Panel>;
