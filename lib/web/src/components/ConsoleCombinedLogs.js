    ...
    const renderConsoleCombinedLogs = () => {
      return combinedLogs.map((log) => {
        ...
        if (level === 'error' || level === 'alert') {
          header = <p className='log_panel_header meta-log'><span className='log_timestamp'>{occurredAt}</span><span className='log_message_top' style={{ color: '#db4e09' }}>{message}</span></p>;
        } else {
          header = <p className='log_panel_header meta-log'><span className='log_timestamp'>{occurredAt}</span><span className='log_message_top'>{message}</span></p>;
        }
