...
class Sidebar extends React.Component {
  constructor (props) {
    ...
    this.state = {
      updates: null,
      startTime: this.getFromLocalStorage('startTime', null), // Store session start time
      isPopUpVisible: false,
      showFeedbackMenu: this.getFromLocalStorage('showFeedbackMenu', false),
      status: this.getFromLocalStorage('status', null), // Keep status for "Remind Me Later"
      pausedAt: null // Store time when tab was switched
  ...
  componentDidMount () {
    console.log('Component Mounted - Checking localStorage values');

    let startTime = this.getFromLocalStorage('startTime', null);
    if (!startTime) {
      startTime = Date.now();
      this.setToLocalStorage('startTime', startTime);
      console.log('New Start Time Set:', startTime);
    } else {
      console.log('Existing Start Time:', startTime);
    }
  ...
  startTimer = () => {
    ...
    this.interval = setInterval(() => {
      ...
      console.log(`Elapsed Time: ${elapsedTime} seconds (StartTime: ${this.state.startTime})`);

      if (elapsedTime >= 60 && this.state.status === null) {
        console.log('Triggering Popup - User spent 1 min on tab');
        clearInterval(this.interval);
        this.setState({ isPopUpVisible: true });
      } else if (elapsedTime >= 60 && this.state.status === 'remindMeLater') {
        console.log('Re-triggering Popup - Remind Me Later');
        this.setState({ isPopUpVisible: true });
      }
    }, 1000);
  };
  ...
  handleVisibilityChange = () => {
    if (document.visibilityState === 'hidden') {
      console.log('Tab is inactive - Pausing Timer');
      clearInterval(this.interval);
      this.setState({ pausedAt: Date.now() });
    } else if (document.visibilityState === 'visible') {
      console.log('Tab is active - Resuming Timer');

      if (this.state.pausedAt) {
        const timeAway = Date.now() - this.state.pausedAt;
        console.log('User was away for:', timeAway / 1000, 'seconds');

        // Adjust startTime by removing the inactive duration
        this.setState(
          (prevState) => ({
            startTime: prevState.startTime + timeAway, // Shift forward
            pausedAt: null // Clear paused time
          }),
          () => {
            this.setToLocalStorage('startTime', this.state.startTime);
            this.startTimer();
          }
        );
      } else {
  ...
  resetTimer = () => {
    ...
    this.setState({ startTime: newStartTime, isPopUpVisible: false }, () => {
      this.setToLocalStorage('startTime', newStartTime);
      this.startTimer();
    });
  };

  handleSkip = () => {
    console.log('Skipping Feedback - Stopping Timer');

    clearInterval(this.interval); // Stop the running timer

    this.setState({ status: 'skipped', isPopUpVisible: false, showFeedbackMenu: true }, () => {
  ...
  handleRemindMeLater = () => {
    console.log('Remind Me Later Clicked - Setting Next Reminder');

    const newStartTime = this.state.startTime + 60000; // Add 60 seconds to startTime

    this.setState({ status: 'remindMeLater', isPopUpVisible: false, startTime: newStartTime }, () => {
  ...
  handleCloseModal = () => {
    ...
    this.setState({ isPopUpVisible: false }); // No reset when simply closing modal
  };
      ...
      <>
        <div className='sidebar-container'>
          ...
          {this.state.isPopUpVisible && (
            ...
            >
              <p>Would you like to give us feedback to improve your experience? Share your thoughts <a href='https://forms.gle/1pJ3jrNH2BSd3mu29' target='_blank' rel='noreferrer'>click here</a>.</p>
            </Modal>
          )}
          <div
            className='overlay'
