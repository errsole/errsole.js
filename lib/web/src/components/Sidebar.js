...
class Sidebar extends React.Component {
  ...
  startTimer = () => {
    if (window.location.hostname === 'localhost' || this.state.status === 'submitted') {
      console.log('Timer not started: Running on localhost or already submitted');
    ...
    this.interval = setInterval(() => {
      ...
      if (elapsedTime >= 60) {
        ...
        clearInterval(this.interval); // Stop the timer when pop-up appears
  ...
  handleRemindMeLater = () => {
    console.log('Remind Me Later Clicked - Restarting Timer for another 1 min');
    ...
    this.setState({
      ...
      this.setToLocalStorage('startTime', this.state.startTime);
      this.startTimer(); // Restart timer for another 1 min
    });
  };
