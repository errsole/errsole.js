...
class Sidebar extends React.Component {
  ...
  initFeedbackTimer = () => {
    ...
    //   return; // Don't start feedback logic in localhost
    // }
    ...
    if (savedStatus === 'submitted') {
      // If the user has already submitted feedback, disable the feedback menu.
      this.setState({ showFeedbackMenu: false });
    } else if (savedStatus === 'remindMeLater' && savedTimer >= 1) {
      // If the user chose "remind me later" and at least 1 minute has passed,
      // reset the timer and status to start over.
      ...
      this.setState({ timer: savedTimer, status: savedStatus, showFeedbackMenu }, () => {
        this.startTimer();
      });
    }
  };
    ...
    if (this.state.status !== 'submitted') {
      this.interval = setInterval(() => {
        this.setState(
          ...
          () => {
            console.log('Timer:', this.state.timer); // Log timer value each tick
            this.setToLocalStorage('timer', this.state.timer);

            if (this.state.timer >= 60 && this.state.status === null) {
              console.log('Feedback popup triggered.');
              this.setState({ isPopUpVisible: true });
            } else if (this.state.timer >= 60 && this.state.status === 'remindMeLater') {
              this.resetTimer();
            }
          }
        );
