...
class Sidebar extends React.Component {
  ...
  initFeedbackTimer = () => {
    ...
    console.log(`App loaded - Timer: ${savedTimer}, Status: ${savedStatus}`);

    if (!savedStatus || savedStatus === 'null') {
      console.log('Fresh session detected. Resetting local storage.');
      this.setToLocalStorage('timer', 0);
      this.setToLocalStorage('status', null);
      this.setState({ timer: 0, status: null, showFeedbackMenu: false }, () => {
        this.startTimer();
      });
    } else if (savedStatus === 'remindMeLater' && savedTimer >= 1) {
      console.log("Resetting timer after 'Remind Me Later'");
      this.setState({ timer: 0, status: null, showFeedbackMenu: false }, () => {
        this.setToLocalStorage('timer', 0);
        this.setToLocalStorage('status', null);
        this.startTimer();
      ...
      console.log('Feedback already submitted, stopping timer.');
      this.setState({ showFeedbackMenu: false });
    } else {
      this.setState({ showFeedbackMenu });
      this.startTimer();
    }
  };
      ...
      this.interval = setInterval(() => {
        this.setState(
          ...
          () => {
            ...
            console.log(`Timer Updated: ${this.state.timer} minute(s)`);

            if (this.state.timer >= 1 && this.state.status === null) {
              console.log('Pop-up showed');
              this.setState({ isPopUpVisible: true });
            } else if (this.state.timer >= 1 && this.state.status === 'remindMeLater') {
              console.log("Timer reset after 'Remind Me Later'");
              this.resetTimer();
            }
          }
        );
  ...
  handleSubmit = () => {
    console.log('Submit clicked - Removing Feedback menu permanently');
    clearInterval(this.interval);
    this.setState(
  ...
  handleSkip = () => {
    console.log('Skip clicked - Stopping timer and adding Feedback menu');
    clearInterval(this.interval);
  ...
  handleRemindMeLater = () => {
    console.log('Remind Me Later clicked - Resetting timer');
  ...
  handleCloseModal = () => {
    console.log("Close (X) clicked - Acting as 'Remind Me Later'");
      ...
      <>
        <div className='sidebar-container'>
          ...
          {this.state.isPopUpVisible && (
            ...
            >
              <p>Would you like to give us feedback to improve your experience?</p>
            </Modal>
          )}
          <div
            className='overlay'
