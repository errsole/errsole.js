...
/* Ante UI */
import { Layout, Input, Icon, Button, Collapse, Select, Spin, Switch, Modal, Tooltip, notification } from 'antd';
...
class ConsoleLogs extends React.Component {
  ...
  getConsoleLogs (queryRequest) {
    let filterLogs = this.state.filterLogs;
    if (queryRequest && queryRequest.filterLogs) {
      filterLogs = queryRequest.filterLogs;
    }
    if (!filterLogs) {
      return false;
    }
    ...
    // get latest logs
    if (!queryRequest || !queryRequest.logStartDatetime) {
      const selectedDatetime = this.state.selectedDatetime || null;
      if (selectedDatetime) {
        query.start_datetime = selectedDatetime.toISOString();
      } else {
        query.start_datetime = new Date().toISOString();
        this.setState({
          selectedDatetime: null
        });
      }
      ...
      this.getCurrentConsoleLogs(query, filterLogs);
    }
    // get logs based on query
    if (queryRequest && queryRequest.logStartDatetime && queryRequest.logOrder) {
      query.start_datetime = queryRequest.logStartDatetime;
      ...
      if (queryRequest.logId) {
        query.previous_log_id = queryRequest.logId;
      }
      this.getCurrentConsoleLogs(query, filterLogs);
    }
  }

  getCurrentConsoleLogs (query, type) {
    const self = this;
    if (type === 'errorsLog' || type === 'logs') {
      this.setState({
        consoleLogLoading: true
      });
      if (type === 'errorsLog') query.level = 'error';
      if (type === 'logs') delete query.level;
  ...
  changeLogsFilter (value) {
    ...
    this.getConsoleLogs({ filterLogs: value });
  }
  ...
  loadMoreErrors (logOrder) {
    ...
    if (currentConsoleLogs.length > 0) {
      if (logOrder === 'latest') {
        ...
        timestamp = currentConsoleLogs[currentConsoleLogs.length - 1].attributes.timestamp;
      } else if (logOrder === 'old') {
        logId = currentConsoleLogs[0].id;
        timestamp = currentConsoleLogs[0].attributes.timestamp;
      }
      this.getConsoleLogs({ logStartDatetime: timestamp, logOrder, logId });
    }
  }
  ...
  setSelectedDatetime (searchDate, searchTime) {
    ...
    if (timezone !== 'Local') {
      ...
      if (isBefore) {
        ...
        this.getConsoleLogs({ logStartDatetime: timestamp, logOrder: 'latest' });
      } else {
  ...
  apply () {
    ...
    if (isDateEmpty && isTimeEmpty) {
      ...
      // var selectedDatetime = moment(new Date(searchDate+' '+searchTime))
  ...
  reset () {
    ...
    this.getConsoleLogs({ logStartDatetime: currentTime, logOrder: 'old' });
  }
  ...
  openCombinedLogsModal (errorLogTimestamp) {
  ...
  render () {
    const filterLogs = this.state.filterLogs;
    ...
    const renderConsoleLogs = () => {
      return currentConsoleLogs.map((log) => {
        ...
        let panel;
        if (filterLogs === 'errorsLog') {
          panel = <Panel className='log_panel' header={header} key={logId}><pre className='log_message_detail'><Tooltip placement='topRight' title='View all logs around this error'><Button onClick={this.openCombinedLogsModal.bind(this, occurredAt)} className='view-log-btn'>View Logs</Button></Tooltip>{message}</pre></Panel>;
        } else {
    ...
    return (
      <div className='logs-layout'>
          ...
          <Content>
            <div className='filter_sort p-20'>
              <div className='filter float-l filter-logs'>
                <Select value={filterLogs} style={{ width: 150 }} onChange={this.changeLogsFilter.bind(this)}>
                  <Option value='errorsLog'>Error Logs</Option>
                  <Option value='logs'>Combined Logs</Option>
                </Select>
              </div>
        ...
        <Modal className='view-logs-modal' width='90%' centered closable maskClosable footer={null} visible={combinedLogsModalStatus} onCancel={this.closeCombinedLogsModal.bind(this, null)} destroyOnClose>
          <ConsoleCombinedLogs errorLogTimestamp={errorLogTimestamp} />
        </Modal>
      </div>
    );
  }
