...
/* Ante UI */
import { DatePicker, Layout, Input, Icon, Button, Collapse, Select, Spin, Switch, Modal, Tooltip, Menu, Dropdown, notification } from 'antd';
  ...
  updateFilterState () {
    ...
    if (filterDetailsFromSession) {
      ...
      if (filterDetails.httpQuery) {
        ...
        if (levelJson) {
          const logsType = JSON.parse(levelJson).map(function (item) {
            return item.source + '.' + item.level + 'Logs';
          });
          this.setState({
            logsType
          });
        } else if (filterDetails?.httpQuery?.logOrder) {
          this.setState({
            logsType: []
          });
        }
