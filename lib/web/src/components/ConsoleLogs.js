...
class ConsoleLogs extends React.Component {
  ...
  getCurrentConsoleLogs (query, logsType) {
    ...
    if (logsType.length > 0) {
      ...
      const allLogsType = ['console.errorLogs', 'console.infoLogs', 'errsole.alertLogs', 'errsole.errorLogs', 'errsole.warnLogs', 'errsole.infoLogs', 'errsole.debugLogs'];
      query.level_json = this.convertLogs(allLogsType);
      query.level_json = JSON.stringify(query.level_json);
    }
    this.setState({
