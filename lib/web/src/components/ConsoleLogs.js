...
/* Ante UI */
import { Layout, Input, Icon, Button, Collapse, Select, Spin, Switch, Modal, Tooltip, Menu, Dropdown, Descriptions, notification, message } from 'antd';
      ...
      <div className='logs-layout'>
        ...
        <Modal className='view-log-details-modal' width='60%' centered closable maskClosable footer={null} visible={logDetailsModalStatus} onCancel={this.closeLogDetailsModal.bind(this, activeLogId)} destroyOnClose>
          <Descriptions title='Log Details' bordered>
            <Descriptions.Item label='Hostname' span={2}>{activeLogDetails.hostname}</Descriptions.Item>
            <Descriptions.Item label='PID'>{activeLogDetails.pid}</Descriptions.Item>
            <Descriptions.Item label='Source' span={2}>{activeLogDetails.source}</Descriptions.Item>
            <Descriptions.Item label='Level'>{activeLogDetails.level}</Descriptions.Item>
          </Descriptions>
        </Modal>
      </div>
    );
