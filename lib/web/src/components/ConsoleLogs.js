...
class ConsoleLogs extends React.Component {
  ...
  getConsoleLogs (queryRequest) {
    ...
    // search time
    if (query.gt_id && this.state.searchGteTimestamp) {
      query.gte_timestamp = queryRequest.datetime ? new Date(queryRequest.datetime).toISOString() : this.state.searchGteTimestamp;
    }
    if (query.lt_id && this.state.searchLteTimestamp) {
      query.lte_timestamp = queryRequest.datetime ? new Date(queryRequest.datetime).toISOString() : this.state.searchLteTimestamp;
    }
      ...
      if (!err) {
        try {
          ...
          if (!filters || Object.keys(filters).length === 0) {
            ...
            self.setState({
              searchGteTimestamp: null,
              searchLteTimestamp: null
            });
          }
          // handle search
          if (filters && Object.keys(filters).length !== 0) {
            let searchGteTimestamp = filters.gte_timestamp;
            let searchLteTimestamp = filters.lte_timestamp;
            if (logs.length === 0) {
              self.notificationMsg('info', 'No searched logs found in the 24-hour interval.');
            } else if (logs.length > 0) {
              logs = self.sortLogs(logs);
              searchLteTimestamp = logs[0].attributes.timestamp;
              searchGteTimestamp = logs[logs.length - 1].attributes.timestamp;
              self.setState({
                currentConsoleLogs: logs
              });
            }
  ...
  loadMoreErrors (logOrder) {
    ...
    if (currentConsoleLogs.length > 0) {
      if (logOrder === 'latest') {
        ...
        if (this.state.searchGteTimestamp || this.state.searchLteTimestamp) {
  ...
  loadNextSearch (logOrder) {
    const currentConsoleLogs = this.state.currentConsoleLogs || [];
    if (currentConsoleLogs.length === 0) {
      const datetime = this.state.searchGteTimestamp;
      this.getConsoleLogs({ datetime, logOrder: 'old' });
    }
  }
  ...
  render () {
    ...
    const searchGteTimestamp = this.state.searchGteTimestamp || null;
    const searchLteTimestamp = this.state.searchLteTimestamp || null;
    ...
    const renderConsoleLogs = () => {
      return currentConsoleLogs.map((log) => {
        ...
        if (level === 'error' || level === 'alert') {
          header = <p className={(meta && meta !== '{}' && 'log_panel_header panel-details') || 'log_panel_header panel-details-no-meta'}><span className='log_timestamp'>{occurredAtFormated}</span><span className='log_message_top' style={{ color: '#db4e09' }}>{message}</span></p>;
        } else {
          header = <p className={(meta && meta !== '{}' && 'log_panel_header panel-details') || 'log_panel_header panel-details-no-meta'}><span className='log_timestamp'>{occurredAtFormated}</span><span className='log_message_top'>{message}</span></p>;
        }
    ...
    let searchGteTimestampFormated, searchLteTimestampFormated;
    if (searchGteTimestamp) {
      searchGteTimestampFormated = timezone === 'Local' ? moment(searchGteTimestamp).format('MMMM Do YYYY HH:mm:ss') : moment.utc(searchGteTimestamp).format('MMMM Do YYYY HH:mm:ss');
    }
    if (searchLteTimestamp) {
      searchLteTimestampFormated = timezone === 'Local' ? moment(searchLteTimestamp).format('MMMM Do YYYY HH:mm:ss') : moment.utc(searchLteTimestamp).format('MMMM Do YYYY HH:mm:ss');
    }
          ...
          <Content>
            ...
            <div className='filter_sort p-20 console_log_content'>
              <div className='content-box'>
                ...
                {currentConsoleLogs.length === 0 && searchGteTimestamp && searchLteTimestamp &&
                  <Collapse activeKey={activeKeys} onChange={this.handlePanelChange.bind(this)}>
                    <p className='logs_more'>No searched logs found in the <b>{searchGteTimestampFormated} - {searchLteTimestampFormated}</b> interval. <a onClick={this.loadNextSearch.bind(this, 'old')}>Load Previous 24 hrs</a></p>
                  </Collapse>}
