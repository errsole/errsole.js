...
/* Ante UI */
import { Layout, Input, Icon, Button, Collapse, Select, Spin, Switch, Modal, Tooltip, Menu, Dropdown, Descriptions, notification } from 'antd';
...
class ConsoleLogs extends React.Component {
  ...
  render () {
    ...
    const renderConsoleLogs = () => {
      return currentConsoleLogs.map((log) => {
        ...
        if (level === 'error' || level === 'alert') {
          header = <p className='log_panel_header  main-logs'><span className='log_timestamp'>{occurredAtFormated}</span><span className='log_message_top' style={{ color: '#db4e09' }}>{message}</span></p>;
        } else {
          header = <p className='log_panel_header  main-logs'><span className='log_timestamp'>{occurredAtFormated}</span><span className='log_message_top'>{message}</span></p>;
        }

        const menu = (
          <Menu>
            <Menu.Item key='1' onClick={this.viewMeta.bind(this, logId)}>
              View Meta
            </Menu.Item>
            <Menu.Item key='2' onClick={this.openLogDetailsModal.bind(this, logId)}>
              View Details
            </Menu.Item>
          </Menu>
        );
    ...
    const showLogDetails = () => {
      if (currentConsoleLogs.length > 0 && activeLogId) {
        const log = currentConsoleLogs.filter((log) => log.id === activeLogId);
        if (log.length > 0) {
          const hostname = log[0].attributes.hostname || 'N/A';
          const pid = log[0].attributes.pid || 'N/A';
          const source = log[0].attributes.source || 'N/A';
          const level = log[0].attributes.level || 'N/A';
          return (
            <Descriptions title='Log Details' bordered>
              <Descriptions.Item label='Hostname' span={2}>{hostname}</Descriptions.Item>
              <Descriptions.Item label='PID'>{pid}</Descriptions.Item>
              <Descriptions.Item label='Source' span={2}>{source}</Descriptions.Item>
