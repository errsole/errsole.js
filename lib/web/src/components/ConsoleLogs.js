    ...
    const renderConsoleLogs = () => {
      return currentConsoleLogs.map((log) => {
        ...
        if (log.attributes.latestTimestamp || log.attributes.oldTimestamp) {
          const oldTimestamp = timezone === 'Local' ? moment(log.attributes.oldTimestamp).format('YYYY-MM-DD HH:mm:ss Z') : moment.utc(log.attributes.oldTimestamp).format('YYYY-MM-DD HH:mm:ss Z');
          const latestTimestamp = timezone === 'Local' ? moment(log.attributes.latestTimestamp).format('YYYY-MM-DD HH:mm:ss Z') : moment.utc(log.attributes.latestTimestamp).format('YYYY-MM-DD HH:mm:ss Z');

          const panel = <Panel className='log_panel' key={Math.random()} header={'No logs were found during the period from ' + oldTimestamp + ' to ' + latestTimestamp}><pre className='log_message_detail'>{'No logs were found during the period from ' + oldTimestamp + ' to ' + latestTimestamp}</pre></Panel>;
          return panel;
        }
