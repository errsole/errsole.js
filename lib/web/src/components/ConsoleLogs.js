...
/* Ante UI */
import { Layout, Input, Icon, Button, Collapse, Select, Spin, Switch, Modal, Tooltip, Menu, Dropdown, notification, message } from 'antd';
...
class ConsoleLogs extends React.Component {
      ...
      if (!err) {
        try {
          ...
          if (!filters || Object.keys(filters).length === 0) {
            if (logs.length === 0) {
              self.notificationMsg('info', 'No logs to load', '', 3);
  ...
  sortLogs (latest) {
    ...
    if (arraysAreEqual(old, uniqueLogs)) {
      this.notificationMsg('info', 'No logs to load', '', 3);
    }
          ...
          <Content>
            ...
            <div className='filter-div'>
              <div className='filter filter-input-div float-l filter-logs'>
                <Select className='filter-input' mode='multiple' placeholder='Select Filter' onChange={this.updateLogsFilter.bind(this)} value={logsType}>
