...
class ConsoleLogs extends React.Component {
  ...
  handleToggleLatestOnTop = (checked) => {
    ...
    const newCombinedConsoleLogs = this.state.currentConsoleLogs;
    newCombinedConsoleLogs.reverse();
    this.setState(() => ({
      latestOnTop: newSortOrder,
      currentConsoleLogs: newCombinedConsoleLogs
    }));
  };
  ...
  getCurrentConsoleLogs (query) {
    ...
    this.setState({ consoleLogLoading: true, abortController, showCancelButton: false });
        ...
        if (!err) {
          try {
            ...
            if (filters && Object.keys(filters).length !== 0) {
              ...
              if (logs.length === 0) {
                logs = [{ type: 'nologs', attributes: { oldTimestamp, latestTimestamp } }];
              }
              logs = this.combineLogs(logs, logOrder, latestOnTop);
              logs = this.removeDuplicateLogs(logs);
              ...
              // set oldTimestamp, or latestTimestamp
              if (!logOrder || (!this.state.oldTimestamp && !this.state.latestTimestamp)) {
                this.setState({
                  oldTimestamp,
                  latestTimestamp
                });
  ...
  loadMoreErrors (logOrder) {
    ...
    if (currentConsoleLogs.length > 0) {
      if (logOrder === 'latest') {
        ...
        } else if (this.state.latestTimestamp) {
        ...
        } else if (this.state.oldTimestamp) {
  ...
  render () {
    ...
    const renderConsoleLogs = () => {
      return currentConsoleLogs.map((log) => {
        if ((!log.attributes || !log.attributes.message || !log.attributes.level || !log.attributes.timestamp) && (!log.attributes.latestTimestamp || !log.attributes.oldTimestamp)) {
          return false;
                  ...
                  <DatePicker
                    ...
                    renderExtraFooter={() => (
                      <div className='date-picker-message'>
                        <InfoCircleOutlined /> Logs generated before this timestamp will be displayed.
                      </div>
                    )}
                  />
                </ConfigProvider>
