...
/* Ante UI */
import { Layout, Divider, Form, Input, Row, Col, Button, Tabs, Result, Select, Spin, message, Tag } from 'antd';
...
class Integrations extends React.Component {
  ...
  addEmailDetails () {
    ...
    const invalidEmails = this.validateEmails(selectedUsers);
    if (invalidEmails.length > 0) {
      message.error(`Invalid email: ${invalidEmails.join(', ')}`);
      return;
    }
                      ...
                      <>
                        <Form layout='horizontal'>
                          <Form.Item>
                            ...
                            <Select className='input-class-margin' name='receivers' mode='tags' placeholder="Select Recipient's Email Address" value={selectedUsers} onChange={this.handleChange.bind(this)} style={{ width: '400px' }}>
                    ...
                    {emailDetails && !editEmailDetails && (
                      <div className='email-integration-container'>
                        <div className='email-field'>
                          <b>Sender Email:</b>
                          <Tag color='green' closable={false} className='email-tag'>
                            {emailDetails.sender || 'Not configured'}
                          </Tag>
                        </div>
                        ...
                        <div className='email-field'>
                          ...
                          <div className='recipient-tags'>
                            {emailDetails.receivers?.split(',').map((email, index) => (
                              <Tag key={index} color='blue' closable={false} className='recipient-tag'>
                                {email.trim()}
                              </Tag>
                            ))}
                          </div>
