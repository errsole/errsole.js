    ...
    if (type === 'slack') {
      this.props.appActions.testSlackNotification(function (err, result) {
        if (!err && result?.attributes?.success) {
          message.success('Test notification sent. If not delivered, please verify slack url is correct.');
        } else {
          message.error('Test notification failed. Please verify slack url is correct.');
        }
        self.setState({
      ...
      this.props.appActions.testEmailNotification(function (err, result) {
        if (!err && result?.attributes?.success) {
          message.success('Test notification sent. If not delivered, please verify smtp details are correct.');
        } else {
          message.error('Test notification failed. Please verify smtp details are correct.');
        }
        self.setState({
          testNotificationLoading: false
        });
