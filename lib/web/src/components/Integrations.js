...
/* Ante UI */
import { Layout, Divider, Form, Input, Row, Col, Button, Icon, Tabs, Result, message } from 'antd';
              ...
              <Content className='setting-user'>
                ...
                <Tabs tabPosition='left'>
                  <TabPane tab={<span><Icon type='slack' />Slack</span>} key='2'>
                    ...
                    {slackDetails && <Result status='success' title={slackStatus ? 'Slack Integrated' : 'Slack Notifications Paused'} subTitle={slackStatus ? 'You will receive notifications for app downtime and custom alerts directly on Slack.' : 'You have paused Slack notifications for app downtime and custom alerts.'} extra={[<Button type='primary' onClick={this.updateSlackDetails.bind(this, !slackStatus)} key='1'> {slackStatus ? 'Pause Notifications' : 'Resume Notifications'} </Button>, <Button type='danger' onClick={this.deleteSlackDetails.bind(this)} key='2'> Remove </Button>]} />}
                  </TabPane>
                  <TabPane tab={<span><Icon type='mail' />Email</span>} key='3'>
                    ...
                    {emailDetails && <Result status='success' title={emailStatus ? <Icon type='check-circle' theme='filled' /> : 'Email Notifications Paused'} subTitle={emailStatus ? 'You will receive notifications for app downtime and custom alerts directly on Email.' : 'You have paused Email notifications for app downtime and custom alerts.'} extra={[<Button type='primary' onClick={this.updateEmailDetails.bind(this, !emailStatus)} key='1'> {emailStatus ? 'Pause Notifications' : 'Resume Notifications'} </Button>, <Button type='danger' onClick={this.deleteEmailDetails.bind(this)} key='2'> Remove </Button>]} />}
                  </TabPane>
                </Tabs>
              </Content>
            </Content>
