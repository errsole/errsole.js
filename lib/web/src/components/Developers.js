...
/* Ante UI */
import { Layout, Divider, Form, Input, Row, Col, Button, Card, Spin, Icon, message } from 'antd';
    ...
    const showAllUsers = () => {
      return allUsers.map((user) => {
        ...
        if (isAdmin && userRole === 'developer') {
          actionBtns = [<Spin key={id} spinning={removeUserLoader}><span onClick={this.removeUser.bind(this, id)}><Icon type='delete' /> Remove</span></Spin>];
        }
        return (
            ...
            <Content className='all-users'>
              <Content>
                ...
                {isAdmin &&
                  <Form layout='inline'>
                    <Form.Item>
                      <Input value={addUserEmail} style={{ width: '300px' }} name='addUserEmail' prefix={<Icon type='user' style={{ color: 'rgba(0,0,0,.25)' }} autocomplete='off' />} placeholder='Developer Email' onChange={this.handleFormChange.bind(this)} />
                      <Input.Password value={addUserPassword} style={{ marginLeft: '16px', width: '300px' }} name='addUserPassword' prefix={<Icon type='lock' style={{ color: 'rgba(0,0,0,.25)' }} autocomplete='off' />} placeholder='Password' onChange={this.handleFormChange.bind(this)} />
                    </Form.Item>
                    <Form.Item>
