...
/* Ante UI */
import { Layout, Divider, Form, Input, Row, Col, Button, message, Popconfirm } from 'antd';
...
class DataRetention extends React.Component {
  ...
  deleteAllLogs () {
    ...
    this.props.logActions.deleteAllLogs(function (err) {
      if (!err) {
          ...
          <Col span={20} offset={2}>
            <Content className='data-retention'>
              <Content>
                ...
                <Divider />
                <p>
                  If needed, you can truncate all logs from the database by clicking the Delete button. This action will remove all log entries.
                  <Popconfirm
                    ...
                    cancelText='No'
                  >
                    <Button type='primary' danger loading={loadingStatus}>Delete</Button>
                  </Popconfirm>
                </p>
              </Content>
            </Content>
