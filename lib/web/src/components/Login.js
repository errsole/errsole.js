...
class Login extends React.Component {
  constructor (props) {
    ...
    this.state = {
      loadingStatus: false,
      storedUserName: window.localStorage.getItem('errsole-user-name') || 'admin'
  ...
  componentDidMount () {
    ...
    this.setState({ storedUserName: window.localStorage.getItem('errsole-user-name') || 'admin' });
    if (isJWTExist) {
    ...
    this.props.userActions.register({ name, email, password, role: 'admin' }, function (err, response) {
      if (!err) {
        if (response && response.attributes && response.attributes.token) {
          window.localStorage.setItem('errsole-user-name', name);
  ...
  render () {
    ...
    const { storedUserName } = this.state;
    const name = this.state.name || '';
    const email = this.state.email || '';
    const password = this.state.password || '';
              ...
              {totalUsers > 0 &&
                <Form layout='vertical' onKeyDown={(event) => event.key === 'Enter' && this.login()}>
                  ...
                  <Form.Item>
                    ...
                    <InfoCircleOutlined className='info-class' /> {` If you don’t have login credentials, please contact @${storedUserName}`}
